...
// Note: Admin initialization is now handled at server startup (see bottom of file)
...
app.delete('/make-server-7c025e45/admin/clear-properties', requireAuth, adminRateLimit, async (c) => {
  try {
    ...
    if (allProperties && allProperties.length > 0) {
      ...
      await kv.mdel(propertyKeys);
      console.log(`✅ Deleted ${propertyKeys.length} properties`);
    }
    
    // Delete all inspections
    if (allInspections && allInspections.length > 0) {
      const inspectionKeys = allInspections.map(i => `inspection:${i.id}`);
      await kv.mdel(inspectionKeys);
      console.log(`✅ Deleted ${inspectionKeys.length} inspections`);
    }
    
    // Delete all financial records
    if (allFinancialRecords && allFinancialRecords.length > 0) {
      const financialKeys = allFinancialRecords.map(f => `financial:${f.id}`);
      await kv.mdel(financialKeys);
      console.log(`✅ Deleted ${financialKeys.length} financial records`);
    }
    
    // Delete all invoices
    if (allInvoices && allInvoices.length > 0) {
      const invoiceKeys = allInvoices.map(i => `invoice:${i.id}`);
      await kv.mdel(invoiceKeys);
      console.log(`✅ Deleted ${invoiceKeys.length} invoices`);
    }
    ...
    return c.json({ 
      success: false, 
      error: `Failed to clear properties: ${error.message}`
    }, 500);
  }
});

app.post('/make-server-7c025e45/admin/initialize', async (c) => {
  try {
    console.log('Manual admin initialization requested...');
    await initializeAdmin();
    
    // Double-check that admin now exists
    const existingUsers = await kv.getByPrefix('user:');
    const adminUser = existingUsers.find(user => 
      user.email === 'r.depala86@gmail.com' && user.role === 'admin'
    );
    ...
    if (adminUser) {
      ...
      return c.json({ 
        success: false, 
        error: 'Admin user initialization completed but user not found in KV store'
      }, 500);
    }
  } catch (error) {
...
app.get('/make-server-7c025e45/admin/status', async (c) => {
  try {
    ...
    try {
      const { data: users } = await supabase.auth.admin.listUsers();
      authUserExists = users?.users?.some(u => u.email === adminEmail) || false;
    } catch (authError) {
    ...
    return c.json({ 
      ...
      totalKVUsers: existingUsers.length
    });
  } catch (error) {
...
app.post('/make-server-7c025e45/admin/reset', async (c) => {
  try {
    const adminEmail = 'r.depala86@gmail.com';
    console.log('Resetting admin user...');
    
    // Remove admin from KV store
    const existingUsers = await kv.getByPrefix('user:');
    const adminUser = existingUsers.find(user => 
      user.email === adminEmail && user.role === 'admin'
    );
    ...
    if (adminUser) {
      await kv.del(`user:${adminUser.id}`);
      console.log('Admin user removed from KV store');
    }
    
    // Try to delete from Supabase Auth (optional, might fail if user doesn't exist)
    try {
      const { data: users } = await supabase.auth.admin.listUsers();
      const authUser = users?.users?.find(u => u.email === adminEmail);
      if (authUser) {
        await supabase.auth.admin.deleteUser(authUser.id);
        console.log('Admin user removed from Supabase Auth');
      }
    } catch (authError) {
      console.log('Could not remove from Auth (may not exist):', authError.message);
    }
    ...
    return c.json({ 
      success: true, 
      message: 'Admin user reset and recreated successfully'
    });
  } catch (error) {
    console.error('Error resetting admin:', error);
    return c.json({ 
      success: false, 
      error: `Failed to reset admin user: ${error.message}`
    }, 500);
  }
});

app.post('/make-server-7c025e45/admin/remove-unauthorized', async (c) => {
  try {
    const unauthorizedEmail = 'rahuldepala4ever@gmail.com';
    console.log('Removing unauthorized admin user:', unauthorizedEmail);
    ...
    const existingUsers = await kv.getByPrefix('user:');
    const unauthorizedUser = existingUsers.find(user => 
      user.email === unauthorizedEmail
    );
    
    if (unauthorizedUser) {
      await kv.del(`user:${unauthorizedUser.id}`);
      console.log('Unauthorized user removed from KV store:', unauthorizedEmail);
    }
    ...
    try {
      ...
      if (authUser) {
        await supabase.auth.admin.deleteUser(authUser.id);
        console.log('Unauthorized user removed from Supabase Auth:', unauthorizedEmail);
      }
    } catch (authError) {
      console.log('Could not remove from Auth (may not exist):', authError.message);
    }
    ...
    return c.json({ 
      success: true, 
      message: `Unauthorized admin user ${unauthorizedEmail} removed successfully`
    });
  } catch (error) {
    console.error('Error removing unauthorized admin:', error);
    return c.json({ 
      success: false, 
      error: `Failed to remove unauthorized admin user: ${error.message}`
    }, 500);
  }
});

// Health check
app.get('/make-server-7c025e45/health', (c) => {
